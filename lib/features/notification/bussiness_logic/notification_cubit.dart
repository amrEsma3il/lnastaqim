...
class NotificationCubit extends Cubit<NotificationState> {
  ...
  void _loadNotificationStates() async {
    ...
    final azkarNotificationFrequancy =
        prefs.getInt('morningAndEviningNotificationFrequancy') ?? 15;
    final salahNabiNotificationFrequancy =
        prefs.getInt('salahNabiNotificationFrequancy') ?? 15;

    final salahNabiNotificationStatus =
        prefs.getBool('salahNabiNotificationStatus') ?? false;
    final morningAndEviningNotificationStatus =
        prefs.getBool('morningAndEviningNotificationStatus') ?? false;
    ...
    final salahNabiNotificationSound =
        prefs.getString('salahNabiNotificationSoundState') ?? 'صلي علي محمد';
    final fajarAlarmSound =
        prefs.getString('fajarAlarmSoundState') ?? 'احمد الطرابلسي';
    final duharAlarmSound =
        prefs.getString('duharAlarmSoundState') ?? 'علي بن احمد الملا';
    final asrAlarmSound =
        prefs.getString('asrAlarmSoundState') ?? 'علي بن احمد الملا';
    final maghribAlarmSound =
        prefs.getString('maghribAlarmSoundState') ?? 'علي بن احمد الملا';
    final ishaAlarmSound =
        prefs.getString('ishaAlarmSoundState') ?? 'علي بن احمد الملا';
  ...
  void onChangeSliderEvent({
    int? azkarNotificationFrequancy,
  ...
  Future<void> changeSalahNabiNotificationStatus() async {
    ...
    emit(
      state.copyWith(
        salahNabiNotificationStatus: !state.salahNabiNotificationStatus,
      ),
    );

    await prefs.setBool(
      'salahNabiNotificationStatus',
      state.salahNabiNotificationStatus,
    );
  ...
  Future<void> changeAzkarNotificationStatus() async {
    emit(
      state.copyWith(
        morningAndEviningNotificationStatus:
            !state.morningAndEviningNotificationStatus,
      ),
    );
  ...
  Future<void> changeSoundSalahNabi() async {
    ...
    await prefs.setString(
      'salahNabiNotificationSoundState',
  ...
  Future<void> registerAllNotifications() async {
    log("registerAllNotifications");
    {
      if (state.fajarAlarmStatus) {
        await _workManagerService.registerFajrParyerTimeTask();
      }
