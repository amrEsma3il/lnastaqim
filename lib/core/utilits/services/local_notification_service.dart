...
class LocalNotificationService {
  ...
  Future<void> showMediaNotification({
    ...
    final String actionId =
        isPlaying ? '${keyFeature}pause' : '${keyFeature}play';
  ...
  Future<void> downloadNotification({
    ...
    // final String playPauseLabel = isPlaying ? 'Pause' : 'Play';
    final String actionId = isPlaying ? 'ايقاف' : 'استئناف';
    ...
    NotificationDetails details = NotificationDetails(
      android: AndroidNotificationDetails(
        ...
        sound: RawResourceAndroidNotificationSound(
          'ahmed_eltrabolsy_fajr'.split('.').first,
        ),
      ),
    );
    ...
    if (currentTimeZone.hour >= 4 && currentTimeZone.hour < 12) {
      ...
      filteredAzkar =
          azkarNotifications
              ...
              .toList();
    } else if ((currentTimeZone.hour >= 19 && currentTimeZone.hour <= 24) ||
        (currentTimeZone.hour >= 1 && currentTimeZone.hour < 3)) {
      // Evening Azkar
      filteredAzkar =
          azkarNotifications
  ...
  Future<void> scheduleNotificationCancellationAt({
    ...
    Duration delayAfterDisplay = const Duration(minutes: 3),
  }) async {
    // final currentTime = tz.TZDateTime.now(tz.local);
    final cancelNotificationDelay =
        scheduledTime.difference(currentTime) + delayAfterDisplay;
    ...
    Future.delayed(cancelNotificationDelay, () async {
      ...
      log(
        'Notification with ID $id has been cancelled after ${delayAfterDisplay.inMinutes} minutes.',
      );
    });
  }
